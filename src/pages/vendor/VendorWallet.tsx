...
const VendorWallet: React.FC = () => {
  ...
  const fetchWalletData = async () => {
    try {
      const { response, data } = await apiRequest('/api/accounts/wallet/');
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const { response, data } = await apiRequest(`/api/accounts/wallet/transactions/?${params}`);
  ...
  const verifyPayment = async (pidx: string) => {
    try {
      const { response, data } = await apiRequest('/api/accounts/wallet/khalti-verify/', {
        method: 'POST',
  ...
  const handleRecharge = async () => {
    ...
    if (selectedPaymentMethod === 'khalti') {
      try {
        ...
        const { response, data } = await apiRequest('/api/accounts/wallet/khalti-payment/', {
          method: 'POST',
          body: JSON.stringify({
